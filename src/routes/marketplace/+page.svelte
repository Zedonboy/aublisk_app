<script lang="ts">
    ...
    let nfts_in_marketplace = []
    let products_on_sale = []
    identity.subscribe((value) => {
        if(!value) return
        let task = async () => {
            let market_actor = createMarketPlaceActor(MARKETPLACE_CANISTER_ID, {agent: new HttpAgent({
                host: CANISTER_HOST, identity: value
            })})

            let list = await market_actor.list_nfts()
            nfts_in_marketplace = list
            products_on_sale = await market_actor.get_nfts_on_sale()
        }
    ...
    <div class="lg:flex 2xl:gap-12 gap-10 2xl:max-w-[1220px] max-w-[1065px] mx-auto" id="js-oversized">
        ...
        <div class="flex-1">
            ...
            <div class="lg:max-w-[680px] w-full">
                ...
                <!-- list products -->
                <div class="grid sm:grid-cols-3 grid-cols-3 gap-3 pb-12">
                    {#each products_on_sale as product}
                    <li class="w-full">
                        <div class="card">
                            ...
                            <div class="card-footer p-2">
                                <button class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Buy</button>
                            </div>
                        </div>
                    </li>
                    {/each}
        ...
        <!-- sidebar -->
        <div class="2xl:w-[380px] lg:w-[330px] w-full mt-[--m-top]">
            ...
            <div class="box p-5 px-6">
                ...
                <div class="flex items-baseline justify-between text-black dark:text-white">
                    <h3 class="font-bold text-base">Your contents waiting to be sold</h3>
                    <a href="#" class="text-sm text-blue-500">See all</a>
                </div>

                <div class="side-list">

                    {#each nfts_in_marketplace as tokendata }
