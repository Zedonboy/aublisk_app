...
>
	<div id="editorjs" class="w-2/3"></div>
	<div class="w-1/3">
		<aside
			id="default-sidebar"
			class="fixed mt-[--m-top] top-0 right-0 z-40 w-64 transition-transform -translate-x-full sm:translate-x-0"
			aria-label="Sidenav"
		>
			<div
					...
					<li>
						<button
							on:click={async (e) => {
								...
								let id = content.blocks[0].id;
								let request = window.indexedDB.open('aublisk_db');
								request.onsuccess = (ev) => {
									db = ev.target.result;
									let store = db
										.transaction(['article_store'], 'readwrite')
										.objectStore('article_store');
									if (saved_id) {
										store.put({ id: saved_id, content });
									} else {
										try {
								...
								request.onupgradeneeded = (ev) => {
									...
									const article_store = db.createObjectStore('article_store', { keyPath: 'id' });
								};
							}}
